...
g2b_point <- g2_no_gis %>%
  ...
  filter(Plot %notin% ELI_2007$Standardized_plot_name)

g2_no_gis <- g2_no_gis %>%
  filter(not.na(UTM_4_Easting))


############# Seperate out plots that are lines instead of points or polygons

g2c_line <- g2_no_gis %>%
  filter(Plot %in% c("SOI-0860", "SOI-0915"))

g2_no_gis <- g2_no_gis %>%
  filter(Plot %notin% c("SOI-0860", 
                        "SOI-0915"))
...
saveRDS(g2b_point, "Robjects/g2b_point.RDS")
saveRDS(g2c_line, "Robjects/g2c_line.RDS")
