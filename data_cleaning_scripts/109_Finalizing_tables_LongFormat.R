...
#version with alert polygons as convex hulls
all_lines_LL <- good_g2c

#create a id column for for merging plot names and coordinates
all_lines_LL$L2 <- 1:nrow(all_lines_LL)

#remove coordinates from the geometry column
all_lines_coords <- data.frame(st_coordinates(all_lines_LL)) %>%
  rename(L2 = L1)

#remove the geometry column
all_lines_LL <- as.data.frame(all_lines_LL) %>%
  select(-geometry)

#Merge plot names and coordinates
all_lines_LL <- merge(all_lines_LL, all_lines_coords) %>%
  select(-L2) %>%
  distinct() %>%
  rename(Plot = Plot, Longitude = X, Latitude = Y)


all_lines_LL <- all_lines_LL %>%
  group_by(Plot) %>%
  mutate(Coordinate_Order = 1:n()) %>%
  ungroup()
...
#version with alert lines as convex hulls
all_lines_utm <- good_g2c

#add utm zones
all_lines_utm  <- merge(all_lines_utm, all_zones)


# create a list of all utm zones in the dataset and their relevent crs
utms83 <- sort(unique(all_lines_utm$UTM_Zone))
...
# split into a list where each element is the plots in one utm zone
lines_by_utm83 <- group_split(all_lines_utm , UTM_Zone)
names(lines_by_utm83) <- utms83



#give each element of the list it's appropriate CRS
for(i in 1:length(utms83)){
  lines_by_utm83[[i]] <- st_transform(lines_by_utm83[[i]], utm_crs83[[i]])
}


#create a id column for for merging plot names and coordinates
lines_by_utm83 <- lapply(lines_by_utm83, function(x) mutate(x, L2 = 1:nrow(x)))

#remove coordinates from the geometry column
coords_by_utm83 <- lapply(lines_by_utm83, function(x) data.frame(st_coordinates(x)))
coords_by_utm83 <- lapply(coords_by_utm83, function(x) rename(x, L2 = L1))

#remove the geometry column
lines_by_utm83 <- lapply(lines_by_utm83, as.data.frame)
lines_by_utm83 <- lapply(lines_by_utm83, select, -geometry)

#Merge plot names and coordinates
for(i in 1:length(lines_by_utm83)){
  lines_by_utm83[[i]] <- merge(lines_by_utm83[[i]], coords_by_utm83[[i]]) %>%
    select(-L2) %>%
    distinct()}

#rename X and Y
lines_by_utm83 <- lapply(lines_by_utm83, function(x) rename(x, Plot = Plot, UTM_Zone = UTM_Zone, UTM_Easting = X, UTM_Northing = Y))




#merge all UTM groups
all_lines_utm <-  rbind(lines_by_utm83[[1]], lines_by_utm83[[2]])


all_lines_utm <- all_lines_utm %>%
  group_by(Plot) %>%
  mutate(Coordinate_Order = 1:n()) %>%
  ungroup()
...
all_LL <- rbind(all_polygons_LL, all_points_LL, all_lines_LL)

all_UTM <- rbind(all_points_utm, all_polygons_utm, all_lines_utm)


all_coordsLONG <- merge(all_UTM, all_LL)


add_missing <- all_coordsLONG[1:length(all_missing$Plot),] 
add_missing$Plot <- all_missing$Plot
add_missing[,2:7] <- NA
  
all_coordsLONG <- rbind(all_coordsLONG, add_missing)  
  
write.csv(all_coordsLONG, "exported/all_coords/all_coordsLONG_2020_09_03.csv")
