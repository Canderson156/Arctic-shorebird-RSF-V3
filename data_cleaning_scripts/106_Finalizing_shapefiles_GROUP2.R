...
g2b_point <- readRDS("Robjects/g2b_point.RDS")
g2c_line <- readRDS("Robjects/g2c_line.RDS")
...
ELI_2007 <- read.csv("data/PRISM/2007_Ellesmere.csv")

ELI_2007 <- ELI_2007 %>%
  select(Plot = Standardized_plot_name,
         Lat,
         Long,
         Order = Lat_Long_Order) %>%
  arrange(Plot, Order)

#need to convert to NAD83
...
st_crs(good_g2c) <- WGS84

good_g2c <- st_transform(good_g2c, NAD83)
...
st_crs(g2c_lineX) <- CRS("+init=EPSG:26715")

g2c_lineX <- st_transform(g2c_lineX, crs = NAD83)


#combine both groups together
good_g2c <- rbind(good_g2c, g2c_lineX)
...
saveRDS(good_g2c, "Robjects/goood_g2c.RDS")
